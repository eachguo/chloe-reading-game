...
# --------------------------
# 核心：复用《安妮的绿山墙》稳定音频播放函数（解决提示音问题）
# --------------------------
def play_feedback_sound(is_correct):
    """
    播放对错提示音，和《安妮的绿山墙》逻辑完全一致
    :param is_correct: True=正确提示音，False=错误提示音
    """
    sound_url = correct_audio_url if is_correct else wrong_audio_url
    components.html(f"""
        <script>
            // 兼容浏览器自动播放策略，确保用户交互后正常播放
            const audio = new Audio("{sound_url}");
            audio.currentTime = 0; // 重置播放进度，避免重复点击时无声音
...
# --------------------------
# 互动选择题：按钮式交互，1:1复刻《安妮的绿山墙》（提示音正常播放）
# --------------------------
